...
const pokemonNamesGreaterThan99 = pokémon.filter( p => p.id > 99).map(p => p.name);
// console.log(pokemonNamesGreaterThan99);

const pokemonPoison = pokémon.filter( p => p.types.length === 1).filter(p => p.types.includes("poison")).map(p => p.name);
// console.log(pokemonPoison);

const secondFlyingType = pokémon.filter(p => p.types[1]==="flying").map(p => p.types[0]);
